...
- [meal/macro tip 2]`;
        ...
        {ticks.map((t) => {
          ...
          return (
            <div key={t} className="flex flex-col items-center gap-1 shrink-0">
              <span className={`text-[7px] font-mono leading-none ${isSelected ? "text-emerald-450 font-bold" : "text-gray-600"}`}>
                {t % 10 === 0 ? t : ""}
              </span>
              <div className={`w-0.5 rounded-full transition-all ${isSelected ? "h-4 bg-emerald-500" : t % 10 === 0 ? "h-2.5 bg-gray-600" : "h-1.5 bg-gray-850"}`} />
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      {/* HEADER */}
      <header className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 backdrop-blur-md shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Vision Active</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <span>{profile.age} yrs • {profile.weight} kg • {profile.height} cm</span>
      ...
      {/* CHAT VIEW SECTION */}
      <section className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col justify-between min-h-[350px] shadow-md backdrop-blur-md">
        ...
        <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1 pb-4 scrollbar-none">
              ...
              <div key={msg.id} className={`flex gap-3 max-w-3xl ${isUser ? "ml-auto flex-row-reverse" : "mr-auto"} animate-slideUp`}>
                ...
                <div className="space-y-2">
                  ...
                  {msg.isPlan && msg.planData && (
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 shadow-sm space-y-4 animate-slideUp w-full max-w-lg">
          ...
          {loading && (
            <div className="flex gap-3 max-w-3xl mr-auto animate-fadeIn">
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 shadow-lg ${theme.iconBg} ${theme.iconText}`}>
                {mode === "roast" ? "😈" : "💪"}
              </div>
              <div className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-4 py-2.5 rounded-2xl rounded-tl-none shadow-md">
          ...
          <div className="flex flex-wrap gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 bg-white/5 hover:bg-white/10 active:scale-95 text-[10px] font-bold text-gray-300 hover:text-white rounded-lg border border-white/10 shadow-lg transition-all shrink-0 cursor-pointer flex items-center gap-1`}
              >
